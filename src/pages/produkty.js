...
const Products = () => {
  ...
  const [posts, setPosts] = useState(data.files.nodes)
  const [currentPage, setCurrentPage] = useState(1)
  const [postsPerPage] = useState(14)
   const [searchValue, setSearchValue] = useState('');
 
  const allCategories = ['Wszystkie Produkty',...new Set(posts.map((item) => item.category))];
  ...
  const filterItems = (category) => {
    if(category === 'Wszystkie Produkty') {
      setPosts(data.files.nodes);
      setCurrentPage(1)
      setCatMenuMobile(!catMenuMobile)
      return;
    }
    const newItems = data.files.nodes.filter((item) => item.category === category);
    setPosts(newItems);
    setCurrentPage(1);
    setCatMenuMobile(!catMenuMobile)
    window.scroll(0,0);
   ...
   const handleSearchInput = (e) => {
    ...
    const newItems = posts.filter((post) => post.title.toLowerCase().includes(e.target.value.toLowerCase()));
      ...
      <StyledPageContainer>
        <StyledPanel>
          <div className="selectCategory">
            ...
            <ul
              className={catMenuMobile ? 'CategoryBoxDesktop' : 'CategoryBoxMobile'}
            >
          ...
          <form className="searchForm">
            ...
            />
            {/* <button onClick={handleSearchButton} type="submit">Szukaj</button> */}
          </form>
        </StyledPanel>
...
const getProducts = graphql`
  {
    files: allContentfulProduct(sort: { fields: createdAt }) {
      nodes {
        capacity
        category
        id
        ...
        image {
          fluid (maxHeight: 300, maxWidth: 200, quality: 100){
